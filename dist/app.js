...
var limpar = function limpar(x) {
  x.value = "";
  x.style.color = "black"; //rexibindo caso tenha dado erro no percorrimento do array
  //removendo msg erro
  ...
  if (erros.length > 0) {
    ...
    document.getElementById("result4").innerHTML = "Pesquise"; //rexibindo box de pesquisas
...
var busca = function busca() {
  ...
  var valor = input.value.toUpperCase(); //se não tiver vazio ou nulo

  if (valor != '' && valor != null) {
    //loader
    var loader = document.getElementById("spinner");
    loader.classList.add("loader");
    var x = new XMLHttpRequest(); //requisição assyncrona

    x.open('GET', 'https://api.jsonbin.io/b/5eeb858919b60f7aa95cc5f2', true); //quando completar a requisição
    ...
    var callback = function callback(y) {
      var achou = false; //percorrer encomendas com o valor do input e comparar

      y.encomendas.forEach(function (element) {
        if (valor == element.numero) {
          encomendaPesquisada = element; //sets nos elements

          document.getElementById("result1").innerHTML = encomendaPesquisada.id + ' - ' + encomendaPesquisada.cliente.nome; //passando valor para string para botar filtro para reais
      ...
      loader.classList.remove("loader"); //se nao nenhum nada
